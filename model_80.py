...
df = pd.read_csv('H:\TechnoColabs Projects\Spotify Project\Main Code\dataset-of-80s.csv')
######################################################

#Shuffling The Data
df = df.sample(frac=1)
#(df.shape) =====> rows=6908, columns=19 
#################################################

X = df.drop(['target', 'track', 'artist', 'uri'], axis=1)
#X.shape ==> rows = 6908, cols = 15
Y = df['target']
hit_flop_count = Y.value_counts()
#hits(1)=3454; flops(0)=3454 ==========> Data is balanced
Y = Y.values
####################################################
...
model_80.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# second hid)den layer
model_80.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# third hidd)en layer
...
fit_model = model_80.fit(x_train, y_train, epochs=20, validation_data=(x_val, y_val), batch_size=100)
...
tf.keras.models.save_model(model_80, 'Trained_model_80')
