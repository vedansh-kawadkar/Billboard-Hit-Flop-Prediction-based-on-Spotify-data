...
df = pd.read_csv('H:\TechnoColabs Projects\Spotify Project\Main Code\dataset-of-10s.csv')
######################################################
...
#hits(1)=3198; flops(0)=3198 ==========> Data is balanced
Y = Y.values
####################################################
...
model_10.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# second hid)den layer
model_10.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# third hidd)en layer
...
fit_model = model_10.fit(x_train, y_train, epochs=20, validation_data=(x_val, y_val), batch_size=100)
...
tf.keras.models.save_model(model_10, 'Trained_model_10')
