...
df = pd.read_csv('H:\TechnoColabs Projects\Spotify Project\Main Code\dataset-of-00s.csv')
######################################################

#Shuffling The Data
df = df.sample(frac=1)
#(df.shape) =====> rows=5872, columns=19 
#################################################

X = df.drop(['target', 'track', 'artist', 'uri'], axis=1)
#X.shape ==> rows = 5872, cols = 15
Y = df['target']
hit_flop_count = Y.value_counts()
#hits(1)=2936; flops(0)=2936 ==========> Data is balanced
Y = Y.values
####################################################
...
model_00.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# second hid)den layer
model_00.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# third hidd)en layer
...
fit_model = model_00.fit(x_train, y_train, epochs=20, validation_data=(x_val, y_val), batch_size=100)
...
tf.keras.models.save_model(model_00, 'Trained_model_00')