...
df = pd.read_csv('H:\TechnoColabs Projects\Spotify Project\Main Code\dataset-of-70s.csv')
######################################################

#Shuffling The Data
df = df.sample(frac=1)
#(df.shape) =====> rows=7766, columns=19 
#################################################

X = df.drop(['target', 'track', 'artist', 'uri'], axis=1)
#X.shape ==> rows = 7766, cols = 15
Y = df['target']
hit_flop_count = Y.value_counts()
#hits(1)=3883; flops(0)=3883 ==========> Data is balanced
Y = Y.values
####################################################
...
model_70.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# second hid)den layer
model_70.add(tf.keras.layers.Dense(nn, activation=tf.nn.relu))# third hidd)en layer
...
fit_model = model_70.fit(x_train, y_train, epochs=20, validation_data=(x_val, y_val), batch_size=100)
...
tf.keras.models.save_model(model_70, 'Trained_model_70')
